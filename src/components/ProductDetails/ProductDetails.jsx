...
// import style from "./ProductDetails.module.css"; // Removed unused import
...
export default function ProductDetails() {
  ...
  function getAllproduct() {
    axios.get(`https://ecommerce.routemisr.com/api/v1/products`)
      .then((res) => {
        // --- FIX START ---
        // Ensure res.data.data exists and is an array before filtering
        if (Array.isArray(res.data.data)) {
          ...
          setRelatedProducts([]); // Set to empty array to prevent further errors
        }
        // --- FIX END ---
      })
      .catch((error) => {
        console.error("Error fetching related products:", error);
        setRelatedProducts([]); // Ensure state is reset on error
      });
  }
  ...
  return (
    <>
      {/* Main Product Details Section - Current styling as per your last request */}
      {Product ?
          ...
          <div className="w-full md:w-[70%] space-y-3 lg:pt-5 text-start md:pl-10">
            ...
            <div className="flex items-center justify-center md:justify-between pt-4 w-full gap-8 md:gap-2">
              <button
                ...
                className="bg-green-600 text-white py-2 px-8 font-serif rounded-md w-[90%] opacity-100 scale-100 transition-all duration-300 hover:bg-green-700"
              >
        ...
        : ( // Added a loading spinner for the main product if not loaded
          ...
          <div key={product.id} className="relative bg-white rounded-lg shadow-md overflow-hidden transition-all duration-300 transform hover:shadow-xl hover:-translate-y-2 hover:scale-[1.02] group">
            <Link to={`/ProductDetails/${product.id}/${product.category.name}`} >
              <div className="relative h-72 sm:h-80 w-full overflow-hidden cursor-pointer"
                // onClick={() => handleProductClick(product)} // handleProductClick is not defined, removed
              >
            ...
            <div className="p-4 flex flex-col justify-between flex-grow">
              ...
              <div className="flex items-center w-full mt-auto pt-2 gap-2">
                <button onClick={(e) => {
                  e.stopPropagation();
                  addTocart(product.id); // Corrected function call as per previous fix
                }}
